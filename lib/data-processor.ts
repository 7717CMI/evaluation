  ...
  const filtered = data.filter((record) => {
    ...
    if (!geoMatch) {
      ...
      const regionToCountriesMap: Record<string, string[]> = geographyCountries || {
        'US': ['Northeast', 'Southwest', 'West', 'Southeast', 'Midwest']
      }

      // If a region is selected and this record is a sub-region in that region, include it
      ...
      if (record.is_aggregated === true) {
        ...
        if (selectedLevel1Segments.length > 0) {
          if (isRegionalSegmentType) {
            const regionalGeographies = ['US', 'Global']
...
export function prepareIntelligentMultiLevelData(
  ...
  records.forEach(record => {
    ...
    if (isRegionalSegmentType && hasExplicitLevel1Selection) {
      ...
      // Check if selected segments are geography names or sub-region names
      ...
      if (selectedAreGeographies && !selectedAreSegments) {
        // All selections are geography names (US, etc.)
        // Group by geography
